...
export default function ScoringView() {
  ...
  useEffect(() => {
    ...
    announceShooter(`Moment ${stageIndex + 1}, ${stageName}`, shooterName, muted)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stageIndex, shooterName])
  ...
  const handlePrevShooter = () => {
    navigate(`/scoring/${stageIndex}/${encodeURIComponent(shooters[prevShooterIndex])}`)
  }
  ...
  return (
    <Layout
      title={`Moment ${stageIndex + 1} — ${shooterName}`}
      backTo="/"
      rightAction={
        <Link
      ...
      <div className="p-4 space-y-3 max-w-xl mx-auto pb-32">
        ...
        <div className="bg-slate-800 rounded-xl p-3 flex items-center gap-3">
          ...
          <div className="flex-1 text-center">
            ...
            <div className="text-xs text-slate-500">
              {currentShooterIndex + 1} av {shooters.length}
            </div>
          </div>
          <button
        ...
        {isDQ && (
          <div className="bg-red-900/30 border border-red-800 rounded-xl p-3 text-center">
            <span className="text-red-400 font-semibold">DISKVALIFICERAD</span>
        ...
        <div className="bg-slate-800 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-slate-200">Träffar</h3>
            <div className="flex gap-3 text-xs text-slate-500">
              <span>M1: {targetTotals[0]}/{shotCounts[0]}</span>
              <span>M2: {targetTotals[1]}/{shotCounts[1]}</span>
            </div>
          </div>
        ...
        <div className="bg-slate-800 rounded-xl p-4">
          <h3 className="font-semibold text-slate-200 mb-3">
            {timeCount === 1 ? 'Tid' : `Tider (${timeCount} serier)`}
          </h3>
          <div className="flex gap-2">
            {Array.from({ length: timeCount }, (_, i) => (
              <div key={i} className="flex-1">
                {timeCount > 1 && (
                  <label className="text-xs text-slate-500 block mb-1">Serie {i + 1}</label>
                )}
                <input
                  type="text"
                  inputMode="decimal"
          ...
          <p className="text-xs text-slate-500 mt-2">Ange i sekunder (t.ex. "10.28" eller "1028")</p>
        </div>
        ...
        <div className={`rounded-xl p-4 ${
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div>
              ...
              <div className="text-xs text-slate-400">Poäng</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-slate-100">{formatTime(totalTime)}</div>
              <div className="text-xs text-slate-400">Tid</div>
            </div>
            <div>
              <div className={`text-2xl font-bold ${hf >= 1.3 ? 'text-green-400' : hf > 0 ? 'text-yellow-400' : 'text-slate-400'}`}>
                {totalTime > 0 ? formatHF(hf) : '—'}
              </div>
              <div className="text-xs text-slate-400">Träfffaktor</div>
            </div>
          </div>
          {status === 'complete' && (
            <div className="mt-2 text-center text-sm text-green-400 font-medium">Moment klart</div>
          )}
        </div>
          ...
          <div>
            {showDQ ? (
              <div className="bg-red-900/20 border border-red-800 rounded-xl p-4 space-y-3">
                <h3 className="font-semibold text-red-400">Diskvalificera {shooterName}</h3>
                <input
                  type="text"
        ...
        >
          {currentShooterIndex === shooters.length - 1
            ? stageIndex < 4
              ? `Nästa moment (Moment ${stageIndex + 2})`
              : 'Avsluta'
            : `Nästa skytt (${shooters[nextShooterIndex]})`}
        </button>
      </div>
    </Layout>
  )
